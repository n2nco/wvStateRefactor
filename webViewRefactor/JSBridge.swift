...
open class JSBridge {
    ...
    internal let jbcontext: JBContext
    ...
    //let app_state = SceneDelegate.app_state
    
    //Alternative: pass in reference to app_state_instnace, then add wkwebview to it, then render this in contentview.
   
    //var app_state_instance = AppState.app_state_instance

    
    public init(libraryCode: String, customOrigin: URL? = nil, headless: Bool = false, incognito: Bool = false) {
        self.jbcontext = JBContext(libraryCode: libraryCode, customOrigin: customOrigin, incognito: incognito)
        self.headless = headless
        self.webView = headless ? nil : self.jbcontext.webView // test this as headless = true some time?
        //b
        ...
        if headless {
            switch globalUIHook {
                case .none: break
                case .view(let view): view.addSubview(self.jbcontext.webView)
                case .viewController(let viewController): viewController.view.addSubview(self.jbcontext.webView)
                case .window(let window): window.addSubview(self.jbcontext.webView)
            }
        }
        #endif
    }
    ...
    internal func call(function: String, withStringifiedArgs args: String) -> Promise<Void> {
        return firstly {
            jbcontext.rawCall(function: function, args: args)
        }.then { _ in
